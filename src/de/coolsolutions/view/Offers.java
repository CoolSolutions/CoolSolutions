	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		try
		{
			...
			if (userID == 0)
			{
				out.println(
						"<html><head><link rel=\"stylesheet\" href=\"https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/"
								...
								+ " crossorigin=\"anonymous\"><title>Unsere " + max
								+ " Angebote für Sie</title></br>&nbsp&nbsp&nbsp");
				...
				} else
				{
					for (int in : indexesOfEntities)
					{
						...
						if (rs.next())
						{
							...
							out.println("<tr class=\"info\"><td>&nbsp" + rs.getString(2)
									+ "<a href=http://localhost:8080/CoolSolutions/Template?articleID="
									+ rs.getString(1) + ">Details</a>" + "</td></tr>");
			...
			} else
			{
				...
				// Die Artikelgruppen, die schon bestellt wurden, in einer
				// ArrayList<String> speichern
				...
				out.println(
						"<html><head><link rel=\"stylesheet\" href=\"https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/"
								...
								+ "<a href=http://localhost:8080/CoolSolutions/Warenkorb?userID=" + userID
								+ ">zum Warenkorb(" + numberOfOrderedArticles + ")</a>&nbsp&nbsp&nbsp&nbsp&nbsp"
								+ "<a href=\" " + response.encodeURL("/CoolSolutions/Logout")
								+ "\">Abmelden</a>&nbsp&nbsp&nbsp&nbsp"
				...
				// Artikel aus max. 5 verschieden (beliebigen) Artikelgruppen
				// anbieten
				ArrayList<Integer> showedGroup = new ArrayList<>();
				for (int numberOfGroups = 0; numberOfGroups < listOfGroups.size()
						&& numberOfGroups < 5; numberOfGroups++)
				{

					// out.println("</br>listOfGroups.size(): " +
					// listOfGroups.size());
					// out.println("</br>numberOfGroups: " + numberOfGroups);
					// out.println("</br>alreadyOrderedAsArrayList.size(): " +
					// alreadyOrderedAsArrayList.size());
					...
					// out.println("</br>listOfGroups.get(groupIndex): " +
					// listOfGroups.get(groupIndex)); // TEST
					SQL = "SELECT Name, ID FROM Artikel WHERE Artikelgruppe_ID =" + listOfGroups.get(groupIndex);
					stmt = conn.createStatement();
					rs = stmt.executeQuery(SQL);

					// Anzahl der Artikel in der Artikelruppe bestimmen und
					// anspeichern
					...
					// out.println("</br>numberOfArticles: " +
					// numberOfArticles);
					// out.println("</br>groupNumber: " + groupNumber);
					// out.println("</br>listOfGroups.size(): " +
					// listOfGroups.size());
					// out.println("</br>listOfGroups.get(0): " +
					// listOfGroups.get(0));
					// out.println("</br>articleNumber: " + articleNumber);

					// Pruefen, ob ein Artikel aus dieser Artikelgruppe schon
					// angezeigt wurde
					...
					if (notYetShowed)
					{
						...
						for (int index = 1; index <= numberOfArticles && rs.next(); index++)
						{
							if (index == articleNumber)
							{
								out.println("<form>");
								out.println("<table class=table table-bordered>");
								out.println("<tr class=\"info\"><td>&nbsp" + rs.getString(1)
										+ "<a href=http://localhost:8080/CoolSolutions/Template?articleID="
										+ rs.getString(2) + ">Details</a>" + "</td></tr>");
								out.println("<tr><td></td></tr>");
								out.println("</table>");
								out.println("</form>");
							}
